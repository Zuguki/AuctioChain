...
const PageBet: FC<IPageBet> = ({ close, lot, setBet }) => {
    ...
    const { error, blurError, postData } = usePostAPI<IPostBet>(
        (dataBet: IPostBet) => LotService.postBetInLot(dataBet),
    );
    ...
    const submitBet = async (): Promise<void> => {
        ...
        const res = await postData(dataUser);
        if (res) {
            setBet(res);
        }
    };
            ...
            <Form className={styleBet.form} onSubmit={submitBet}>
                <LogicDownload isLoading={isLoading}>
                    <>
                        ...
                        <h3>Ставка</h3>
                        <LogicFormProcessing loading={isLoading} err={error} />
